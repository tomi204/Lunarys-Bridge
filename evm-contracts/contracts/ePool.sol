...
contract EPool is SepoliaConfig, Ownable, ReentrancyGuard {
    ...
    function finalizeSwap(
        ...
        if (zeroForOne) {
            uint256 r0AfterTmp = r0Before + inAfterFee;
            ...
            _allowHandle(outHandle, address(token1));
            _allowHandle(outHandle, recipient);
            FHE.allowTransient(outHandle, address(token1));
            token1.confidentialTransfer(recipient, outHandle);
        } else {
            uint256 r1AfterTmp = r1Before + inAfterFee;
            r1After = r1AfterTmp;
            ...
            euint64 outHandle = FHE.asEuint64(_cast64(outClear));
            _allowHandle(outHandle, address(token0));
            _allowHandle(outHandle, recipient);
            FHE.allowTransient(outHandle, address(token0));
            token0.confidentialTransfer(recipient, outHandle);
        }
    ...
    function provideLiquidityToken0(
        ...
        uint64 amount64 = _cast64(amount0Clear);

        euint64 amountHandle = FHE.fromExternal(encryptedAmount0, inputProof);
        _allowHandle(amountHandle, address(this));
        _allowHandle(amountHandle, address(positionNFT));
        _allowHandle(amountHandle, recipient);
        _allowHandle(amountHandle, address(token0));

        FHE.allowTransient(amountHandle, address(token0));
        euint64 transferred = token0.confidentialTransferFrom(msg.sender, address(this), amountHandle);
        ...
        _allowHandle(transferred, address(token0));
        ...
        positionLiquidity[tokenId] = amount0Clear;

        uint112 newReserve0 = _cast112(uint256(reserve0Last) + amount0Clear);
    ...
    function claimRewards() external nonReentrant returns (uint256 rewards) {
        ...
        if (rewards > 0) {
            ...
            _allowHandle(rewardHandle, address(token0));
            _allowHandle(rewardHandle, msg.sender);
            _allowHandle(rewardHandle, address(this));
            FHE.allowTransient(rewardHandle, address(token0));
            token0.confidentialTransfer(msg.sender, rewardHandle);

            uint112 newReserve0 = _cast112(poolBal - rewards);
            _updateReserves(newReserve0, reserve1VirtualLast);
        }
        emit RewardsClaimed(msg.sender);
    }
